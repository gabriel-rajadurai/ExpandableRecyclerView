...
public abstract class ExpandableRecyclerViewAdapter<GVH extends GroupViewHolder, CVH extends ChildViewHolder>
    ...
    private boolean shouldOthersCollapse = false;

    /**
     * The position of the current expanded group,
     * This is flatPos(the actual element of the index in the list).
     * A value of -1 means no group is expanded
     */
    protected int expandedGroupPos = -1; //No groups are expanded at the start


    /**
     * constructor to initialize the adapter
     *
     * @param groups               list of ExpandableGroup objects
     * @param shouldOthersCollapse a boolean to specify if other groups should collapse when a group is expanded
     */
    public ExpandableRecyclerViewAdapter(List<? extends ExpandableGroup> groups,
        ...
        this.shouldOthersCollapse = shouldOthersCollapse;
    }
    ...
    public void onAllGroupsCollapsed() {
        //TODO, should the library do anything from its side
        Log.d(TAG, "All groups are collapsed: ");
    }

    /**
     * Triggered by a click on a {@link GroupViewHolder}
     * <p>
     * if shouldOthersCollapse is true, current expanded group will collapse
     * and the newly clicked group will expand
     *
    ...
    public boolean onGroupClick(int flatPos) {
        ...
        if (shouldOthersCollapse) {
            if (isGroupExpanded(flatPos)) {
                expandedGroupPos = -1; // No groups expanded now
                return toggleGroup(flatPos);
            } else {
                if (expandedGroupPos >= 0) {
                    toggleGroup(expandedGroupPos);
                    if (flatPos > expandedGroupPos) {
                        flatPos = flatPos - getGroups().get(expandedGroupPos).getItemCount();
                    }
                }
            }

            expandedGroupPos = flatPos;
        }
        ...
        for (ExpandableGroup group : getGroups())
        {
            ...
            if (isGroupExpanded(group)) {
                ...
                currentGroupPos +=group.getItemCount();

                if(flatPos>currentGroupPos) {
                    flatPos -= group.getItemCount();
                    currentGroupPos = flatPos;
                }
            }
            else {
                currentGroupPos++;
      ...
      /* expandedGroupCount = getGroups().get(flatPos).getItemCount();
       expandedGroupPos = flatPos + expandedGroupCount;

       if(flatPos>expandedGroupPos){
          flatPos-=expandedGroupCount;
          expandedGroupPos = flatPos;
       }*/
        return toggleGroup(flatPos);
    }
     ...
     * Notifies that a particular group specified by thr group position
     * <p>
     * NOTE: Please pass in a valid Group position. Otherwise it will result in crashes.
     *
     * @param groupPos the position of the group removed
     */
    public void notifyGroupRemoved(int groupPos) {
        if (shouldOthersCollapse) expandedGroupPos = -1;
        expandCollapseController.removeState(groupPos);
        notifyItemRemoved(groupPos);
    }

    public void notifyGroupAdded(int groupPos) {

        notifyItemInserted(groupPos);
        expandCollapseController.addState(groupPos);
    }
